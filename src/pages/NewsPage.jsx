...
export default function NewsPage() {
  ...
  return <div className="news-page">
    ...
    <section className="news-header-card">
      ...
      <div className="news-toolbar"><div className="news-toolbar__filters"><div className="news-toolbar__filter-row"><label className="search-field news-toolbar__search"><span className="sr-only">News durchsuchen</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="News durchsuchen" /></label><div className="news-filter-group" aria-label="Ansicht filtern"><span className="news-filter-group__label">Ansicht</span>{priorityFilters.map((filter) => <button key={filter.value} className={priorityFilter === filter.value ? 'news-filter news-filter--active' : 'news-filter'} type="button" onClick={() => setPriorityFilter(filter.value)}>{filter.label}</button>)}</div><div className="news-filter-group" aria-label="Persönliche Merker filtern"><span className="news-filter-group__label">Merker</span><button className={showLater ? 'news-filter news-filter--active' : 'news-filter'} type="button" aria-pressed={showLater} onClick={() => setShowLater((value) => !value)}>Später lesen</button><button className={showFavorites ? 'news-filter news-filter--active' : 'news-filter'} type="button" aria-pressed={showFavorites} onClick={() => setShowFavorites((value) => !value)}>Favoriten</button></div><label className="filter-field news-toolbar__period"><span className="sr-only">Zeitraum filtern</span><select value={period} onChange={(event) => setPeriod(event.target.value)}>{periodFilters.map((filter) => <option key={filter.value} value={filter.value}>{filter.label}</option>)}</select></label></div><div className="news-toolbar__filter-row news-toolbar__filter-row--details"><span className="news-filter-group__label">Weitere Filter</span><NewsMultiSelect label="Länder" options={filterOptions.countries} selected={selectedCountries} onToggle={(value, checked) => toggleSelection(setSelectedCountries, value, checked)} /><NewsMultiSelect label="Tags" options={filterOptions.tags} selected={selectedTags} onToggle={(value, checked) => toggleSelection(setSelectedTags, value, checked)} /><NewsMultiSelect label="Betrifft" options={filterOptions.affects} selected={selectedAffects} onToggle={(value, checked) => toggleSelection(setSelectedAffects, value, checked)} /><button className="news-filter-reset" type="button" disabled={!hasActiveFilters} onClick={resetFilters}><span aria-hidden="true">×</span>Filter zurücksetzen</button></div></div>{category === 'internal' && canEdit('news') && <button className="button" type="button" onClick={() => setEditingItem('new')}>Meldung hinzufügen</button>}</div>
