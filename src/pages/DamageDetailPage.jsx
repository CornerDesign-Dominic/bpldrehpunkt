    ...
    <div className="todo-detail-page damage-detail-page">
      ...
      <div className="todo-detail-layout">
        ...
        <aside className="todo-detail-sidebar">
          <section><DetailSectionHeading onEdit={editable ? () => setEditing('general') : null}>Allgemein</DetailSectionHeading><dl><Detail label="Status"><span className={`todo-status damage-status damage-status--${damageCase.status}`}>{damageCaseStatusLabel(damageCase.status)}</span></Detail><Detail label="Schadenart">{damageCase.damageType}</Detail><Detail label="Schadendatum">{formatDate(damageCase.damageDate)}</Detail><Detail label="Nächste Frist"><span className={dueClass(damageCase)}>{dueValue}</span></Detail><Detail label="Schadenhöhe">{formatCurrency(damageCase.damageAmount)}</Detail><Detail label="Schaden-/Aktennummer BPL-Versicherung">{damageCase.bplInsuranceCaseNumber}</Detail><Detail label="Verantwortliche Person">{damageCase.responsibleUserName}</Detail></dl></section>
          <section><DetailSectionHeading onEdit={editable ? () => setEditing('links') : null}>Verknüpfungen</DetailSectionHeading><dl><Detail label="Auftrag-/Tourreferenz">{damageCase.transportReference}</Detail></dl></section>
          ...
          <section><DetailSectionHeading onEdit={editable ? () => setEditing('liability') : null}>Haftungsgrundlage</DetailSectionHeading><dl><Detail label="Rechtsgrundlage">{labelFor(DAMAGE_LEGAL_BASES, damageCase.legalBasis)}</Detail><Detail label="Gewicht der Ware">{formatNumber(damageCase.cargoWeightKg, ' kg')}</Detail><Detail label="Bemessungs-/Haftungsgrenze">{formatCurrency(damageCase.liabilityLimit)}</Detail><Detail label="Versicherungsrelevanz">{labelFor(DAMAGE_INSURANCE_RELEVANCE, damageCase.insuranceRelevance)}</Detail><Detail label="Bemerkung zur Haftung">{damageCase.liabilityNote}</Detail></dl></section>
