    ...
    <div className="pallet-account-page">
      ...
      {activeForm === 'movement' && <form className="pallet-entry-form pallet-movement-form" onSubmit={handleMovementSubmit}>
        ...
        <section className="pallet-movement-partner-grid">
          <label className="form-field pallet-movement-partner"><span>Kunde</span><select value={movementForm.customerId} onChange={(event) => setMovementForm({ ...movementForm, customerId: event.target.value })}><option value="">Kunde auswählen</option>{customers.map((item) => <option key={item.id} value={item.id}>{item.companyName} · Debitor {item.debtorNumber}</option>)}</select>{selectedCustomer && <small>{selectedCustomer.companyName} · DyCoS Debitor: {selectedCustomer.debtorNumber}</small>}</label>
          <label className="form-field pallet-movement-partner"><span>Unternehmer</span><select value={movementForm.carrierId} onChange={(event) => setMovementForm({ ...movementForm, carrierId: event.target.value })}><option value="">Unternehmer auswählen</option>{carriers.map((item) => <option key={item.id} value={item.id}>{item.companyName} · Kreditor {item.creditorNumber}</option>)}</select>{selectedCarrier && <small>{selectedCarrier.companyName} · DyCoS Kreditor: {selectedCarrier.creditorNumber}</small>}</label>
        </section>
        <section className="pallet-movement-matrix">
          ...
          <div className="pallet-movement-matrix__grid">
            ...
            <strong>Ladestelle</strong><label><span className="sr-only">Ladestelle erhalten</span><input aria-label="Ladestelle erhalten" inputMode="numeric" min="0" step="1" type="number" value={movementForm.loadingPoint.received} onChange={(event) => updateStation('loadingPoint', 'received', event.target.value)} /></label><label><span className="sr-only">Ladestelle abgegeben</span><input aria-label="Ladestelle abgegeben" inputMode="numeric" min="0" step="1" type="number" value={movementForm.loadingPoint.delivered} onChange={(event) => updateStation('loadingPoint', 'delivered', event.target.value)} /></label>
            <strong>Entladestelle</strong><label><span className="sr-only">Entladestelle erhalten</span><input aria-label="Entladestelle erhalten" inputMode="numeric" min="0" step="1" type="number" value={movementForm.unloadingPoint.received} onChange={(event) => updateStation('unloadingPoint', 'received', event.target.value)} /></label><label><span className="sr-only">Entladestelle abgegeben</span><input aria-label="Entladestelle abgegeben" inputMode="numeric" min="0" step="1" type="number" value={movementForm.unloadingPoint.delivered} onChange={(event) => updateStation('unloadingPoint', 'delivered', event.target.value)} /></label>
          </div>
        </section>
        <section className="pallet-movement-result">
          <h4>Ergebnis der Buchung</h4>
          <div>{selectedCarrier && <article><span>Unternehmer</span><strong>{selectedCarrier.companyName}</strong><output>{formatNumber(movementCalculation.carrierBalance, true)} Paletten</output></article>}{selectedCustomer && <article><span>Kunde</span><strong>{selectedCustomer.companyName}</strong><output>{formatNumber(movementCalculation.customerBalance, true)} Paletten</output></article>}</div>
        </section>
        <label className="form-field pallet-movement-note"><span>Bemerkung</span><input value={movementForm.note} onChange={(event) => setMovementForm({ ...movementForm, note: event.target.value })} /></label>
