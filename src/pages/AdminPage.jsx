...
function HolidaySyncLog({ entries }) {
  return <section className="admin-panel admin-holiday-sync-log"><div className="admin-panel__heading"><div><h2>Aktualisierungsprotokoll</h2><p>Die letzten zehn Synchronisationsläufe.</p></div></div>{entries.length ? <div className="admin-holiday-sync-log__entries">{entries.map((entry) => {
    ...
    const missingTranslations = Array.isArray(entry.translationMissingNames) ? entry.translationMissingNames : []
    return <div className="admin-holiday-sync-log__entry" key={entry.id}><div><strong>{date ? syncDateFormatter.format(date) : 'Zeitpunkt wird geladen …'}</strong><span>{entry.trigger === 'automatic' ? 'Automatisch' : entry.actorName || 'Administrator'}</span></div><div className="admin-holiday-sync-log__details"><span className={succeeded ? 'admin-holiday-sync-log__status admin-holiday-sync-log__status--success' : 'admin-holiday-sync-log__status admin-holiday-sync-log__status--failed'}>{succeeded ? (created || changed ? 'Erfolgreich' : 'Erfolgreich – keine Änderungen') : 'Fehlgeschlagen'}</span><span>{created} neu · {changed} geändert</span>{missingTranslations.length > 0 && <span className="admin-holiday-sync-log__translation">Übersetzung fehlt: {missingTranslations.join(', ')}</span>}{!succeeded && entry.errorMessage && <span className="admin-holiday-sync-log__error">{entry.errorMessage}</span>}</div></div>
...
export default function AdminPage() {
  ...
  async function refreshHolidays() {
    ...
    try {
      const result = await httpsCallable(functions, 'refreshHolidayData', { timeout: 180000 })()
