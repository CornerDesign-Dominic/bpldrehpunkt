...
import { businessDays, formatVacationPeriod, requestOverlaps, todayValue } from '../lib/vacationRequests.js'
...
function RequestDetailModal({ request, originalRequest, onClose, onProcess }) {
  ...
  return <div className="vacation-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}><section className="vacation-modal vacation-modal--detail" role="dialog" aria-modal="true" aria-labelledby="vacation-management-detail-title"><div className="vacation-modal__heading"><div><h2 id="vacation-management-detail-title">{actionLabel}</h2><p>{request.employeeName} · {request.employeeDepartment || 'Keine Abteilung'}</p></div><button className="vacation-modal__close" type="button" onClick={onClose} aria-label="Dialog schließen">×</button></div><dl className="vacation-detail-list"><div><dt>Mitarbeiter</dt><dd>{request.employeeName}</dd></div><div><dt>Abteilung</dt><dd>{request.employeeDepartment || 'Keine Abteilung'}</dd></div><div><dt>Antragstyp</dt><dd>{requestTypes[type]}</dd></div><div><dt>Status</dt><dd><span className={`vacation-management-status vacation-management-status--${status}`}>{statusLabels[status]}</span></dd></div><div><dt>Eingereicht am</dt><dd>{submittedAt(request)}</dd></div><div><dt>Urlaubstage</dt><dd>{requestDays(request)}</dd></div>{type === 'request' && <div className="vacation-detail-list__wide"><dt>Beantragter Zeitraum</dt><dd>{formatVacationPeriod(request)}</dd></div>}{type === 'change' && <><div className="vacation-detail-list__wide"><dt>Bisheriger Zeitraum</dt><dd>{formatVacationPeriod(original)}</dd></div><div className="vacation-detail-list__wide"><dt>Neuer gewünschter Zeitraum</dt><dd>{formatVacationPeriod(request)}</dd></div></>}{type === 'cancellation' && <div className="vacation-detail-list__wide"><dt>Zu stornierender Zeitraum</dt><dd>{formatVacationPeriod(original)}</dd></div>}{request.note && <div className="vacation-detail-list__wide"><dt>Notiz</dt><dd>{request.note}</dd></div>}</dl><div className="vacation-modal__actions"><button className="button button--secondary" type="button" onClick={onClose}>Abbrechen</button>{status === 'pending' && <><button className="button button--secondary" type="button" onClick={() => onProcess(request, 'rejected')}>{actionLabel} ablehnen</button><button className="button" type="button" onClick={() => onProcess(request, 'approved')}>{actionLabel} genehmigen</button></>}</div></section></div>
}
...
export default function VacationManagementPage() {
  ...
  function calendarEntriesForDate(value) {
    const baseRequests = filteredRequests.filter((item) => !item.originalRequestId && requestStatus(item) !== 'rejected' && requestOverlaps(item, value, value))
    const changeRequests = filteredRequests.filter((item) => requestType(item) === 'change' && ['pending', 'approved'].includes(requestStatus(item)) && requestOverlaps(item, value, value))
    return [
      ...
      ...baseRequests.map((item) => {
        ...
        return { id: `base-${item.id}`, label: `${item.employeeName.split(' ')[0]}${annotation ? ` · ${annotation}` : ''}`, kind: requestStatus(item), modifier: pendingCancellation ? 'cancellation_requested' : pendingChange ? 'change_requested' : '', title: `${item.employeeName} · ${annotation || statusLabels[requestStatus(item)]}` }
      }),
      ...changeRequests.map((item) => ({ id: `change-${item.id}`, label: `${item.employeeName.split(' ')[0]} · Änderung`, kind: requestStatus(item), modifier: requestStatus(item) === 'pending' ? 'change_requested' : '', title: `${item.employeeName} · ${requestStatus(item) === 'approved' ? 'Änderung genehmigt' : 'Änderung angefragt'}` })),
    ]
  }

  function moveMonth(delta) { const next = new Date(year, month + delta, 1); setYear(next.getFullYear()); setMonth(next.getMonth()) }
  async function process(request, decision) { setProcessingId(request.id); try { await processVacationRequest(request.id, decision); await reload(); setSelectedRequest(null); const actionLabel = requestTypes[requestType(request)] || 'Antrag'; setToast(decision === 'approved' ? `${actionLabel} genehmigt.` : `${actionLabel} abgelehnt.`) } catch { setToast('Der Urlaubsantrag konnte nicht bearbeitet werden.') } finally { setProcessingId('') } }
  function requestProcess(request, decision) { setConfirmation({ request, decision }) }
