...
export default function VacationManagementPage() {
  ...
  const departments = useMemo(() => [...new Map(requests.filter((item) => item.employeeDepartmentId).map((item) => [item.employeeDepartmentId, item.employeeDepartment])).entries()].map(([id, name]) => ({ id, name })).sort((left, right) => left.name.localeCompare(right.name, 'de')), [requests])
  const employees = useMemo(() => [...new Set(requests.filter((item) => department === 'all' || item.employeeDepartmentId === department).map((item) => item.employeeName).filter(Boolean))].sort((left, right) => left.localeCompare(right, 'de')), [department, requests])
  ...
  const visibleRequests = useMemo(() => requests
    ...
    .filter((item) => department === 'all' || item.employeeDepartmentId === department)
