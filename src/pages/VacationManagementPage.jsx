...
export default function VacationManagementPage() {
  ...
  const selectedMonthEnd = `${year}-${String(month + 1).padStart(2, '0')}-${String(new Date(year, month + 1, 0).getDate()).padStart(2, '0')}`
  const visibleRequests = useMemo(() => currentListRequests.filter((item) => [item, item.activeRequest].some((request) => request?.startDate <= selectedMonthEnd && request?.endDate >= selectedMonthStart)).filter((item) => status === 'all' || requestStatus(item) === status).sort((left, right) => {
    if (sort === 'start_desc') return (right.startDate || '').localeCompare(left.startDate || '')
    if (sort === 'newest') return (right.submittedAt || '').localeCompare(left.submittedAt || '')
  ...
  const calendarEntries = useMemo(() => [
    ...
    ...filteredRequests.filter((item) => requestType(item) === 'request' && (status === 'all' || requestStatus(item) === status)).flatMap((item) => {
      ...
      const vacationEntry = { id: `vacation-${item.id}`, startDate: item.startDate, endDate: item.endDate, label: `${item.employeeName.split(' ')[0]}${annotation ? ` · ${annotation}` : ''}`, kind: requestStatus(item), modifier: pendingRequest ? `${requestType(pendingRequest)}_requested` : '', title: `${item.employeeName} · ${annotation || statusLabels[requestStatus(item)]}` }
  ...
  function showRequestInCalendar(request) {
    ...
    const [targetYear, targetMonth] = target.startDate.split('-').map(Number)
    setYear(targetYear)
    setMonth(targetMonth - 1)
    setCalendarHighlight((current) => ({
      focusedDate: target.startDate,
      highlightedEntryId: activeRequest && requestType(activeRequest) === 'change' ? `change-${activeRequest.id}` : `vacation-${request.id}`,
  ...
  return <div className="vacation-management-page">{toast && <Toast message={toast} onDismiss={() => setToast('')} />}<ConfirmDialog open={Boolean(confirmation)} title={confirmation ? `${confirmationType} ${confirmation.decision === 'approved' ? 'genehmigen?' : 'ablehnen?'}` : ''} message={confirmation ? `Den ${confirmationType.toLowerCase()} von ${confirmation.request.employeeName} für ${formatVacationPeriod(confirmation.request)} wirklich ${confirmation.decision === 'approved' ? 'genehmigen' : 'ablehnen'}?` : ''} confirmLabel={confirmation?.decision === 'approved' ? 'Genehmigen' : 'Ablehnen'} submittingLabel="Wird bearbeitet …" variant={confirmation?.decision === 'rejected' ? 'danger' : 'primary'} isSubmitting={Boolean(processingId)} onCancel={() => setConfirmation(null)} onConfirm={() => { const target = confirmation; setConfirmation(null); if (target) process(target.request, target.decision, target.managerComment) }} /><section className="vacation-management-filters"><CalendarNavigation onPrevious={() => moveMonth(-1)} onToday={showToday} onNext={() => moveMonth(1)}><label><span className="sr-only">Monat</span><select value={month} onChange={(event) => setMonth(Number(event.target.value))}>{VACATION_MONTHS.map((label, index) => <option key={label} value={index}>{label}</option>)}</select></label><label><span className="sr-only">Jahr</span><select value={year} onChange={(event) => setYear(Number(event.target.value))}>{years.map((item) => <option key={item} value={item}>{item}</option>)}</select></label></CalendarNavigation><label><span className="sr-only">Abteilung</span><select value={department} onChange={(event) => { setDepartment(event.target.value); setEmployee('all') }}><option value="all">Alle Abteilungen</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span className="sr-only">Mitarbeiter</span><select value={employee} onChange={(event) => setEmployee(event.target.value)}><option value="all">Alle Mitarbeiter</option>{employees.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label><span className="sr-only">Status</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Alle Status</option><option value="pending">Ausstehend</option><option value="approved">Genehmigt</option><option value="rejected">Abgelehnt</option><option value="cancelled">Storniert</option><option value="withdrawn">Zurückgezogen</option></select></label><label><span className="sr-only">Sortierung</span><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="start_asc">Startdatum aufsteigend</option><option value="start_desc">Startdatum absteigend</option><option value="newest">Neueste Vorgänge zuerst</option><option value="oldest">Älteste Vorgänge zuerst</option></select></label></section><div className="vacation-management-layout"><section className="vacation-calendar-card vacation-management-calendar">{loading ? <p className="vacation-state">Kalender wird geladen …</p> : <><VacationCalendar key={calendarHighlight?.token || 'calendar'} year={year} month={month} today={todayValue()} entries={calendarEntries} focusedDate={calendarHighlight?.focusedDate} highlightedEntryId={calendarHighlight?.highlightedEntryId} /><VacationCalendarLegend /></>}</section><aside className="vacation-management-list"><div className="vacation-management-list__heading"><h3>Anträge &amp; Urlaube</h3></div><div className="vacation-management-request-list">{loading ? <p className="vacation-state">Anträge werden geladen …</p> : visibleRequests.length ? visibleRequests.map((request) => { const activeRequest = request.activeRequest && requestStatus(request.activeRequest) === 'pending' ? request.activeRequest : null; const displayedRequest = activeRequest || request; return <article className="vacation-management-request" key={request.id} role="button" tabIndex="0" onClick={() => showRequestInCalendar(request)}><span className="vacation-management-request__top"><strong>{request.employeeName} ({request.employeeDepartment || 'Keine Abteilung'})</strong><span className={`vacation-management-status vacation-management-status--${requestStatus(request)}`}>{statusLabels[requestStatus(request)]}</span></span><span className="vacation-management-request__period">{formatVacationPeriod(displayedRequest)}</span>{activeRequest && <span className="vacation-management-request__substatus"><span className="vacation-management-status vacation-management-status--pending">{requestTypes[requestType(activeRequest)]} · Ausstehend</span></span>}<a className="vacation-management-request__details" href={`#urlaub-${request.id}`} onClick={(event) => { event.preventDefault(); event.stopPropagation(); setSelectedRequest(request) }}>Details</a></article> }) : <p className="vacation-state">Keine Urlaubsanträge für diese Auswahl.</p>}</div></aside></div>{selectedRequest && <RequestDetailModal request={selectedRequest} actionRequest={requestStatus(selectedRequest.activeRequest) === 'pending' ? selectedRequest.activeRequest : null} history={history} onClose={() => setSelectedRequest(null)} onProcess={requestProcess} />}</div>
  return <div className="vacation-management-page">{toast && <Toast message={toast} onDismiss={() => setToast('')} />}<ConfirmDialog open={Boolean(confirmation)} title={confirmation ? `${confirmationType} ${confirmation.decision === 'approved' ? 'genehmigen?' : 'ablehnen?'}` : ''} message={confirmation ? `Den ${confirmationType.toLowerCase()} von ${confirmation.request.employeeName} für ${formatVacationPeriod(confirmation.request)} wirklich ${confirmation.decision === 'approved' ? 'genehmigen' : 'ablehnen'}?` : ''} confirmLabel={confirmation?.decision === 'approved' ? 'Genehmigen' : 'Ablehnen'} submittingLabel="Wird bearbeitet …" variant={confirmation?.decision === 'rejected' ? 'danger' : 'primary'} isSubmitting={Boolean(processingId)} onCancel={() => setConfirmation(null)} onConfirm={() => { const target = confirmation; setConfirmation(null); if (target) process(target.request, target.decision, target.managerComment) }} /><div className="vacation-management-heading"><h2>Urlaubsmanagement</h2></div>{error && <p className="form-error">{error}</p>}<div className="vacation-management-layout"><section className="vacation-calendar-card vacation-management-calendar"><div className="vacation-toolbar"><CalendarNavigation onPrevious={() => moveMonth(-1)} onToday={showToday} onNext={() => moveMonth(1)}><label className="filter-field"><span className="sr-only">Monat</span><select value={month} onChange={(event) => setMonth(Number(event.target.value))}>{VACATION_MONTHS.map((label, index) => <option key={label} value={index}>{label}</option>)}</select></label><label className="filter-field"><span className="sr-only">Jahr</span><select value={year} onChange={(event) => setYear(Number(event.target.value))}>{years.map((item) => <option key={item} value={item}>{item}</option>)}</select></label></CalendarNavigation><div className="vacation-toolbar__filters"><label className="filter-field"><span className="sr-only">Abteilung</span><select value={department} onChange={(event) => { setDepartment(event.target.value); setEmployee('all') }}><option value="all">Alle Abteilungen</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="filter-field"><span className="sr-only">Mitarbeiter</span><select value={employee} onChange={(event) => setEmployee(event.target.value)}><option value="all">Alle Mitarbeiter</option>{employees.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div></div>{loading ? <p className="vacation-state">Kalender wird geladen …</p> : <><VacationCalendar key={calendarHighlight?.token || 'calendar'} year={year} month={month} today={todayValue()} entries={calendarEntries} focusedDate={calendarHighlight?.focusedDate} highlightedEntryId={calendarHighlight?.highlightedEntryId} /><VacationCalendarLegend /></>}</section><aside className="vacation-management-list"><div className="vacation-management-list__heading"><h3>Anträge &amp; Urlaube</h3><label className="filter-field"><span className="sr-only">Status</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Alle Status</option><option value="pending">Ausstehend</option><option value="approved">Genehmigt</option><option value="rejected">Abgelehnt</option><option value="cancelled">Storniert</option><option value="withdrawn">Zurückgezogen</option></select></label></div><div className="vacation-management-request-list">{loading ? <p className="vacation-state">Anfragen werden geladen …</p> : visibleRequests.length ? visibleRequests.map((request) => { const activeRequest = request.activeRequest && requestStatus(request.activeRequest) === 'pending' ? request.activeRequest : null; const displayedRequest = activeRequest || request; return <article className="vacation-management-request" key={request.id} role="button" tabIndex="0" onClick={() => showRequestInCalendar(request)} onKeyDown={(event) => { if (event.target === event.currentTarget && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); showRequestInCalendar(request) } }}><span className="vacation-management-request__top"><strong>{request.employeeName} ({request.employeeDepartment || 'Keine Abteilung'})</strong><span className={`vacation-management-status vacation-management-status--${requestStatus(request)}`}>{statusLabels[requestStatus(request)]}</span></span><span className="vacation-management-request__period">{formatVacationPeriod(displayedRequest)}</span>{activeRequest && <span className="vacation-management-request__substatus"><span className="vacation-management-status vacation-management-status--pending">{requestTypes[requestType(activeRequest)]} · {statusLabels[requestStatus(activeRequest)]}</span></span>}<a className="vacation-management-request__details" href={`#urlaub-${request.id}`} onClick={(event) => { event.preventDefault(); event.stopPropagation(); setSelectedRequest(request) }} onKeyDown={(event) => event.stopPropagation()}>Details</a></article> }) : <p className="vacation-state">Keine Urlaubsanträge für diese Auswahl.</p>}</div></aside></div>{selectedRequest && <RequestDetailModal request={selectedRequest} actionRequest={requestStatus(selectedRequest.activeRequest) === 'pending' ? selectedRequest.activeRequest : null} history={history} onClose={() => setSelectedRequest(null)} onProcess={requestProcess} />}</div>
}
