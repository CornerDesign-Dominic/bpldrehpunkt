...
function RequestModal({ request, onClose, onSubmit }) {
  ...
  const [form, setForm] = useState({ startDate: initialStartDate, endDate: initialEndDate, days: String(request?.days ?? businessDays(initialStartDate, initialEndDate)), vacationType: getVacationType(request?.vacationType).value, note: request?.note || '' })
