...
export default function VacationPage() {
  ...
  const visibleRequests = useMemo(() => requests.filter((request) => {
    ...
    return !request.originalRequestId
      ...
      && (!request.hrManualEntry || request.status !== 'withdrawn')
  ...
  const isSelectedHrManualVacation = selectedBaseRequest?.hrManualEntry === true
  const withdrawableRequest = !isSelectedHrManualVacation && (pendingCancellationRequest || (selectedBaseRequest && getVacationRequestKind(selectedBaseRequest) === 'vacation' && getMainVacationStatus(selectedBaseRequest) === 'pending' ? selectedBaseRequest : null))
  const cancellableRequest = !isSelectedHrManualVacation && (selectedRequest && !isCancellationRequest(selectedRequest) && selectedRequest.status === 'approved' ? selectedRequest : selectedBaseRequest && !isCancellationRequest(selectedBaseRequest) && selectedBaseRequest.status === 'approved' ? selectedBaseRequest : null)
  return <div className="vacation-page">
    {toast && <Toast message={toast} onDismiss={() => setToast('')} />}
    <section className="vacation-calendar-card">
      <div className="vacation-toolbar"><CalendarNavigation onPrevious={() => moveMonth(-1)} onToday={showToday} onNext={() => moveMonth(1)}><label className="filter-field"><span className="sr-only">Monat</span><select value={month} onChange={(event) => setMonth(Number(event.target.value))}>{VACATION_MONTHS.map((label, index) => <option key={label} value={index}>{label}</option>)}</select></label><label className="filter-field"><span className="sr-only">Jahr</span><select value={year} onChange={(event) => setYear(Number(event.target.value))}>{years.map((item) => <option key={item} value={item}>{item}</option>)}</select></label></CalendarNavigation><div className="vacation-toolbar__filters"><label className="filter-field"><span className="sr-only">Kalenderansicht</span><select value={calendarScope} onChange={(event) => setCalendarScope(event.target.value)}><option value="self">Mein Kalender</option>{ownDepartment && <option value="department">Meine Abteilung</option>}</select></label></div></div>
