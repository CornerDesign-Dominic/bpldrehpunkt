...
function NodeEditor({ departments, functionalRoles, node, onCancel, onRemoveOutput, onSave, process }) {
  ...
  return <div className="process-modal-backdrop">
    ...
    <form className="process-modal" onSubmit={save}>
      <div className="process-modal__heading"><h2>{node.type === 'start' ? 'Startblock bearbeiten' : 'Block bearbeiten'}</h2><button className="button button--secondary" type="button" onClick={onCancel}>Schließen</button></div>
...
export default function KnowledgeProcessPage({ isNew = false }) {
  ...
  function connectToNode(targetId) {
    setProcess((current) => ({ ...current, edges: [...current.edges, { id: `edge_${connecting.node.id}_${connecting.outputId}_${targetId}`, sourceId: connecting.node.id, targetId, sourceOutputId: connecting.outputId }] }))
    setConnecting(null)
  }
  ...
  const connectableNodes = connecting ? process.nodes.filter((node) => ['action', 'checklist', 'end'].includes(node.type) && node.id !== connecting.node.id && !canReachNode(node.id, connecting.node.id)) : []
  return <div className="knowledge-process-page">
    ...
    {connecting && <div className="process-modal-backdrop"><section className="process-modal process-connect-picker"><div className="process-modal__heading"><h2>Mit bestehendem Schritt verbinden</h2><button className="button button--secondary" type="button" onClick={() => setConnecting(null)}>Schließen</button></div><p>Wähle einen vorhandenen Schritt. Er wird nicht kopiert und kann von mehreren Antwortwegen genutzt werden.</p><div>{connectableNodes.length ? connectableNodes.map((node) => <button type="button" key={node.id} onClick={() => connectToNode(node.id)}><strong>{node.title || 'Unbenannter Block'}</strong><span>{node.type === 'action' ? 'Handlung' : node.type === 'checklist' ? 'Checkliste' : 'Ende'}</span></button>) : <p className="process-connect-picker__empty">Es gibt keinen passenden Block ohne zyklische Verbindung.</p>}</div></section></div>}
    ...
    <ProcessFlow process={process} editable={editMode} onAdd={(node, outputId) => setAdding({ node, outputId })} onConnect={(node, outputId) => setConnecting({ node, outputId })} onInsert={setInserting} onEdit={setNodeEditing} onDelete={(node) => { const incoming = process.edges.filter((edge) => edge.targetId === node.id).length; const outgoing = process.edges.filter((edge) => edge.sourceId === node.id).length; setConfirmation({ title: 'Block löschen?', message: `Es werden ${incoming} eingehende und ${outgoing} ausgehende Verbindung${outgoing === 1 ? '' : 'en'} entfernt. Nicht mehr erreichbare Folgeschritte werden ebenfalls entfernt; gemeinsame Schritte bleiben erhalten.`, confirmLabel: 'Löschen', variant: 'danger', action: () => deleteNode(node) }) }} />
  </div>
}
