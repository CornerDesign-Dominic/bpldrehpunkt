...
export default function KnowledgeProcessPage({ isNew = false }) {
  ...
  return <div className="knowledge-process-page">
    ...
    <div className="knowledge-process-page__toolbar"><BackLink to="/wissen-prozesse" />{isNew && editMode && <button className="button button--secondary" type="button" onClick={() => { setAiTemplateError(''); setAiTemplateOpen(true) }}>Vorlage mit KI erstellen</button>}{!editMode && canModify && <button className="button" type="button" onClick={() => setEditing(true)}>Prozess bearbeiten</button>}{editMode && <><button className="button button--secondary" type="button" onClick={() => { setEditing(false); if (isNew) navigate('/wissen-prozesse') }}>Abbrechen</button><button className="button button--secondary" type="button" disabled={saving} onClick={() => save('draft')}>Als Entwurf speichern</button><button className="button" type="button" disabled={saving || validationErrors.length > 0} onClick={() => save('active')}>Freigeben</button></>}{editable && process.status === 'active' && !editMode && <button className="button button--secondary" type="button" onClick={() => setConfirmation({ title: 'Prozess archivieren?', message: 'Der Prozess ist anschließend nur noch für Nutzer mit Bearbeitungsrecht sichtbar.', confirmLabel: 'Archivieren', action: () => save('archived') })}>Archivieren</button>}</div>
