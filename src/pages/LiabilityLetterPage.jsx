...
export default function LiabilityLetterPage() {
  ...
  return <>
    ...
    <div className="liability-page">
      ...
      <div className="liability-page__document-actions"><label className={`liability-signature-option${usePersonalSignature ? ' liability-signature-option--active' : ''}`}><input type="checkbox" checked={usePersonalSignature} onChange={(event) => { void togglePersonalSignature(event.target.checked) }} disabled={signatureLoading} /><span className="liability-signature-option__content"><strong>Persönliche Unterschrift verwenden</strong><small>Name und hinterlegte Unterschrift werden im Dokument eingefügt.</small>{signatureLoading && <small>Unterschrift wird geladen …</small>}</span></label><div className="liability-page__actions"><button className="button button--secondary" type="button" disabled={signatureLoading} onClick={() => { void requestPdfAction('print') }}>PDF drucken</button><button className="button" type="button" disabled={isCreatingPdf || signatureLoading} aria-busy={isCreatingPdf} onClick={() => { void requestPdfAction('create') }}>PDF erstellen</button></div></div>
      {signatureNoticeVisible && <div className="liability-signature-notice" role="status"><p>Du hast noch keine persönliche Unterschrift hinterlegt. Bitte hinterlege deine Unterschrift zuerst unter „Mein Profil“.</p><Link className="button button--secondary" to="/profil">Zu Mein Profil</Link></div>}
