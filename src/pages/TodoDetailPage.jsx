...
export default function TodoDetailPage() {
  ...
  useEffect(() => {
    let current = true
    Promise.all([getTodoById(todoId), listTodoUpdates(todoId), editable ? listVisibleUserDirectory() : Promise.resolve([]), canViewMasterData ? listBusinessPartners() : Promise.resolve([]), canViewDamageCases ? listDamageCases() : Promise.resolve([])])
      .then(([todo, todoUpdates, profiles, businessPartners, cases]) => { if (current) { setResult({ todo, error: todo ? '' : 'Aufgabe nicht gefunden.' }); setUpdates(todoUpdates); setUpdatesLoading(false); setUsers(profiles); setPartners(businessPartners); setDamageCases(cases) } })
  ...
  }, [canViewDamageCases, canViewMasterData, editable, todoId])
