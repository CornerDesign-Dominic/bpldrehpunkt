...
export default function TodoDetailPage() {
  ...
  useEffect(() => {
    let current = true
    Promise.all([getTodoById(todoId), listTodoUpdates(todoId), editable ? listVisibleUserDirectory() : Promise.resolve([]), canViewMasterData ? listBusinessPartners() : Promise.resolve([]), canViewDamageCases ? listDamageCases() : Promise.resolve([]), canViewInsolvencies ? listInsolvencies() : Promise.resolve([])])
      .then(([todo, todoUpdates, profiles, businessPartners, cases, insolvencyEntries]) => { if (current) { setResult({ todo, error: todo ? '' : 'Aufgabe nicht gefunden.' }); setUpdates(todoUpdates); setUpdatesLoading(false); setUsers(profiles); setPartners(businessPartners); setDamageCases(cases); setInsolvencies(insolvencyEntries) } })
  ...
  }, [canViewDamageCases, canViewInsolvencies, canViewMasterData, editable, todoId])
  ...
  const linkedDamageCase = canViewDamageCases && todo.damageCaseId ? damageCases.find((damageCase) => damageCase.id === todo.damageCaseId) : null
  const linkedDamageCaseLabel = linkedDamageCase ? [linkedDamageCase.caseNumber, linkedDamageCase.title].filter(Boolean).join(' · ') : ''
  const linkedInsolvency = canViewInsolvencies && todo.insolvencyId ? insolvencies.find((insolvency) => insolvency.id === todo.insolvencyId) : null
  const linkedInsolvencyLabel = linkedInsolvency ? [linkedInsolvency.partnerName, linkedInsolvency.courtReference].filter(Boolean).join(' · ') : ''
  ...
  return <>
    ...
    {quickEditing && <TodoQuickEditModal key={quickEditing} canViewDamageCases={canViewDamageCases} canViewInsolvencies={canViewInsolvencies} currentUserId={user.uid} damageCases={damageCases} insolvencies={insolvencies} partners={partners} section={quickEditing} todo={todo} users={activeUsers} onCancel={() => setQuickEditing(null)} onSubmit={saveQuickEdit} />}
    <div className="todo-detail-layout"><main className="todo-detail-main"><section className="todo-detail-content"><DetailSectionHeading onEdit={canManageSections ? () => setQuickEditing('content') : null}>Beschreibung</DetailSectionHeading><p className="todo-detail-description">{todo.description || 'Keine Beschreibung hinterlegt.'}</p></section><section className="todo-updates" aria-labelledby="todo-updates-title"><div className="todo-updates__heading"><h3 id="todo-updates-title">Updates zum Fall</h3><span>{caseUpdates.length}</span></div>{canView('todos') && <form className="todo-updates__form" onSubmit={saveNote}><textarea aria-label="Update zum Fall" rows="2" value={note} maxLength="1000" onChange={(event) => setNote(event.target.value)} placeholder="Update zum Fall hinzufügen …" /><button className="button" type="submit" disabled={noteSaving || !note.trim()}>{noteSaving ? 'Wird gespeichert …' : 'Update hinzufügen'}</button></form>}{updatesLoading ? <p className="todo-updates__empty">Updates werden geladen …</p> : caseUpdates.length ? <ol className="todo-updates__list">{caseUpdates.map((update) => <li key={update.id} className="todo-updates__item todo-updates__item--note"><div><strong>{update.createdByName}</strong><span>Update · {formatTimestamp(update.createdAt)}</span></div><p>{update.text}</p></li>)}</ol> : <p className="todo-updates__empty">Noch keine Updates zum Fall.</p>}</section><section className="todo-updates todo-history" aria-labelledby="todo-history-title"><div className="todo-updates__heading"><h3 id="todo-history-title">Historie</h3><span>{history.length}</span></div>{updatesLoading ? <p className="todo-updates__empty">Historie wird geladen …</p> : history.length ? <ol className="todo-updates__list">{history.map((update) => <li key={update.id} className={`todo-updates__item todo-updates__item--${update.type}`}><div><strong>{update.createdByName}</strong><span>{update.type === 'note' ? 'Update' : 'System'} · {formatTimestamp(update.createdAt)}</span></div><p>{update.text}</p></li>)}</ol> : <p className="todo-updates__empty">Noch keine Historieneinträge.</p>}</section></main><aside className="todo-detail-sidebar"><section><DetailSectionHeading onEdit={canManageSections ? () => setQuickEditing('schedule') : null}>Priorität &amp; Termine</DetailSectionHeading><TodoPriority priority={todoPriority(todo)} /><dl><Detail label="Fällig am"><span className={dueClass(todo)}>{formatDate(todo.dueDate)}</span></Detail><Detail label="Erinnerung am">{formatDate(todo.reminderDate)}</Detail></dl></section><section><DetailSectionHeading onEdit={canManageSections ? () => setQuickEditing('responsibility') : null}>Zuständigkeit</DetailSectionHeading><dl><Detail label="Zielgruppe">{todo.audienceLabel}</Detail><Detail label="Aktueller Bearbeiter">{todo.assignedUserName || 'Noch nicht übernommen'}</Detail></dl></section><section><DetailSectionHeading onEdit={canManageSections ? () => setQuickEditing('links') : null}>Verknüpfungen</DetailSectionHeading><dl><Detail label="Kunde">{todo.customerId && canViewMasterData ? <Link to={`/kunden-unternehmer/${effectivePartner(todo.customerId)?.id || todo.customerId}`}>{effectivePartner(todo.customerId)?.companyName || todo.customerName || 'Kunde öffnen'}</Link> : todo.customerName}</Detail><Detail label="Unternehmer">{todo.carrierId && canViewMasterData ? <Link to={`/kunden-unternehmer/${effectivePartner(todo.carrierId)?.id || todo.carrierId}`}>{effectivePartner(todo.carrierId)?.companyName || todo.carrierName || 'Unternehmer öffnen'}</Link> : todo.carrierName}</Detail><Detail label="Schadenfall">{linkedDamageCase ? <Link to={`/schaeden/${linkedDamageCase.id}`}>{linkedDamageCaseLabel}</Link> : todo.damageCaseId ? 'Schadenfall nicht verfügbar' : null}</Detail><Detail label="Insolvenz">{linkedInsolvency ? <Link to={`/insolvenzen/${linkedInsolvency.id}`}>{linkedInsolvencyLabel}</Link> : todo.insolvencyId ? 'Insolvenz nicht verfügbar' : null}</Detail><Detail label="TA-Nummer">{todo.reference}</Detail></dl></section><section className="todo-detail-system"><h3>Systemdaten</h3><dl><Detail label="Erstellt von">{todo.creatorName}</Detail><Detail label="Erstellt am">{formatTimestamp(todo.createdAt)}</Detail><Detail label="Zuletzt aktualisiert">{formatTimestamp(todo.updatedAt)}</Detail><Detail label="Übernommen am">{formatTimestamp(todo.assignedAt)}</Detail>{todo.completedAt && <><Detail label="Erledigt am">{formatTimestamp(todo.completedAt)}</Detail><Detail label="Erledigt von">{todo.completedByName}</Detail></>}{todo.withdrawnAt && <><Detail label="Zurückgezogen am">{formatTimestamp(todo.withdrawnAt)}</Detail></>}{todo.withdrawnAt && <Detail label="Zurückgezogen von">{todo.withdrawnByUserId}</Detail>}</dl></section></aside></div>
    </div>
  </>
}
