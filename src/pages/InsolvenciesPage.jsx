...
export default function InsolvenciesPage() {
  ...
  return <div className="damages-page insolvencies-page">
    ...
    {loading ? <p className="page-state">Insolvenzfälle werden geladen …</p> : <div className="todo-sections"><section className="todo-section insolvencies-filter-card" aria-labelledby="insolvencies-filter-heading"><div className="todo-section__heading"><h2 id="insolvencies-filter-heading">Filter</h2></div><div className="damage-filters insolvencies-filters"><label className="search-field"><span className="sr-only">Insolvenzfälle durchsuchen</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Unternehmen, Aktenzeichen oder Gerichtsstand" /></label><label className="filter-field"><span className="sr-only">Eröffnungsjahr</span><select value={year} onChange={(event) => setYear(event.target.value)}><option value="">Alle Eröffnungsjahre</option>{years.map((item) => <option key={item} value={item}>{item}</option>)}</select></label></div></section><section className="todo-section" aria-labelledby="insolvencies-heading"><div className="todo-section__heading"><h2 id="insolvencies-heading">Insolvenzfälle</h2><span>{visibleInsolvencies.length}</span></div><InsolvenciesTable insolvencies={visibleInsolvencies} emptyMessage={insolvencies.length ? 'Keine Insolvenzfälle für diese Filter gefunden.' : undefined} onOpen={(insolvency) => navigate(`/insolvenzen/${insolvency.id}`)} /></section></div>}
