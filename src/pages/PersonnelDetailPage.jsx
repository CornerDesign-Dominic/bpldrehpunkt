...
export default function PersonnelDetailPage() {
  ...
  return <div className="personnel-detail-page">{toast && <Toast message={toast} onDismiss={() => setToast('')} />}<div className="personnel-detail__heading"><BackLink to="/personal" /></div>{error && <p className="form-error">{error}</p>}
    ...
    <section className="personnel-detail__form personnel-vacation-detail"><div className="personnel-vacation-detail__heading"><h2>Urlaubsübersicht</h2><div className="personnel-vacation-detail__filters"><label className="filter-field"><span>Anzeige</span><select value={vacationDisplay} onChange={(event) => setVacationDisplay(event.target.value)}><option value="all">Alle anzeigen</option><option value="relevant">Relevante anzeigen</option></select></label><label className="filter-field"><span>Jahr</span><select value={vacationYear} onChange={(event) => setVacationYear(Number(event.target.value))}>{vacationYears.map((year) => <option key={year} value={year}>{year}</option>)}</select></label></div>{canModify && <div className="personnel-vacation-detail__actions"><button className="button" type="button" onClick={() => { setSelectedManualVacation(null); setManualVacationModalOpen(true) }}>Urlaub hinzufügen</button><button className="button" type="button" onClick={() => setAdjustmentModalOpen(true)}>Ausgleich hinzufügen</button></div>}</div><PersonnelVacationTable vacations={displayedVacations} includeEmployee={false} editable={canModify} onEdit={setSelectedVacation} onEditManualVacation={(vacation) => { setSelectedManualVacation(vacation); setManualVacationModalOpen(true) }} /></section>
