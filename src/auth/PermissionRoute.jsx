...
export default function PermissionRoute({ module, children, requireUserManagement = false, requireVacationManagement = false }) {
  const { isLoading, profile } = useAuth()
  const location = useLocation()
  if (isLoading) return <AuthLoadingScreen />
  const allowed = requireUserManagement ? canManageUsers(profile) : requireVacationManagement ? canManageVacations(profile) : canView(profile, module)
