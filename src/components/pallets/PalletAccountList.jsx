...
export default function PalletAccountList() {
  ...
  return <div className="pallets-page">
    ...
    <div className="table-frame pallets-table-frame"><table><thead><tr><th>Firmenname</th><th>Ort</th><th>DyCoS Debitor</th><th>DyCoS Kreditor</th><th>Saldo</th><th>Letzter Kontoabschluss</th><th>Öffnen</th></tr></thead><tbody>{loading ? <tr><td colSpan="7" className="table-state">Palettenkonten werden geladen …</td></tr> : partnerError ? <tr><td colSpan="7" className="table-state">Keine Geschäftspartner verfügbar.</td></tr> : visibleAccounts.length ? visibleAccounts.map(({ partner, account }) => <tr key={partner.id}><td><strong>{partner.companyName}</strong>{partner.shortName && <span className="table-subline">{partner.shortName}</span>}</td><td>{partner.address?.city || '—'}</td><td>{partner.debtorNumber || '—'}</td><td>{partner.creditorNumber || '—'}</td><td>{accountError ? '—' : formatPalletNumber(account.balance, true)}</td><td>{accountError ? '—' : formatClosing(account.latestClosing)}</td><td className="table-action"><Link to={`/paletten/${partner.id}`}>Öffnen</Link></td></tr>) : <tr><td colSpan="7" className="table-state">Keine Geschäftspartner gefunden.</td></tr>}</tbody></table></div>
  </div>
}
