...
export default function PalletClosingForm({ accountBalance, closingForm, formError, isSubmitting, newClosingBalance, onCancel, onChange, onSubmit }) {
  ...
  return <form className="pallet-entry-form pallet-closing-form" onSubmit={onSubmit}>
    ...
    <section className="pallet-closing-adjustment"><h4>Saldoänderung</h4><div className="pallet-closing-adjustment__content"><div className="pallet-closing-change"><div className="pallet-closing-change__controls"><div className="pallet-closing-direction"><button className={closingForm.direction === 'add' ? 'is-active' : ''} type="button" onClick={() => onChange('direction', 'add')} aria-pressed={closingForm.direction === 'add'}>+ Hinzufügen</button><button className={closingForm.direction === 'subtract' ? 'is-active' : ''} type="button" onClick={() => onChange('direction', 'subtract')} aria-pressed={closingForm.direction === 'subtract'}>− Abziehen</button></div><div className="pallet-closing-quantity-row"><label className="form-field pallet-closing-quantity"><span>Anzahl Paletten</span><input autoFocus inputMode="numeric" min="0" step="1" type="number" value={closingForm.quantity} onChange={(event) => onChange('quantity', event.target.value)} /></label><span className="pallet-closing-unit">Paletten</span></div></div></div><div className="pallet-closing-balances"><div className="pallet-closing-value"><span>Aktueller Saldo</span><strong>{formatAmount(accountBalance)}</strong></div><div className="pallet-closing-value"><span>Neuer Saldo</span><strong>{formatAmount(newClosingBalance)}</strong></div></div></div></section>
