...
export default function PalletAccountPartnerCard({ partner }) {
  ...
  return <section className="pallet-account-partner-card">
    ...
    <div className="pallet-account-partner-card__meta"><span>{getBusinessPartnerType(partner)}</span><span>{address}</span><span>{location}</span><span>{partner.address?.country || '—'}</span><span>DyCoS-Debitor: {partner.debtorNumber || '—'}</span><span>DyCoS-Kreditor: {partner.creditorNumber || '—'}</span></div>
  </section>
}
