...
export default function InsolvencyClaimsOverview({ canEdit, claims, quotaPayments, loading, onSaveClaim, onDeleteClaim, onSaveQuotaPayment, onDeleteQuotaPayment }) {
  ...
  const paymentColumns = canEdit ? 5 : 4
  ...
  return <section className="todo-detail-content insolvency-claims-overview" aria-label="Forderungsübersicht">
    <section className="insolvency-claims-overview__section" aria-labelledby="insolvency-claims-title">
      ...
      <div className="todos-table-frame insolvency-claims-overview__table-frame"><table className="todos-table insolvency-claims-overview__table insolvency-claims-overview__table--claims"><thead><tr><th>Rechnungsnummer</th><th>Netto</th><th>USt.</th><th>Brutto</th><th>Zur Insolvenztabelle angemeldet</th>{canEdit && <th><span className="sr-only">Aktionen</span></th>}</tr></thead><tbody>
    ...
    <section className="insolvency-claims-overview__section insolvency-claims-overview__section--payments" aria-labelledby="insolvency-quota-payments-title">
      ...
      <div className="todos-table-frame insolvency-claims-overview__table-frame"><table className="todos-table insolvency-claims-overview__table insolvency-claims-overview__table--payments"><thead><tr><th>Datum</th><th>Netto</th><th>USt.</th><th>Brutto</th>{canEdit && <th><span className="sr-only">Aktionen</span></th>}</tr></thead><tbody>
        {loading ? <tr><td className="table-state" colSpan={paymentColumns}>Quotenzahlungen werden geladen …</td></tr> : !quotaPayments.length && !draftPayment ? <tr><td className="table-state" colSpan={paymentColumns}>Noch keine Quotenzahlungen erhalten.</td></tr> : quotaPayments.map((payment) => {
          ...
          return <tr key={payment.id} className={editing ? 'insolvency-claims-overview__row--editing' : ''}>{editing ? <><PaymentFields values={draft.values} onChange={updateDraft} /><Actions editing disabled={saving} onSave={saveDraft} onCancel={() => setDraft(null)} /></> : <><td>{formatDate(payment.paymentDate)}</td><td className="insolvency-claims-overview__amount">{formatCurrency(payment.netAmount)}</td><td className="insolvency-claims-overview__amount">{formatCurrency(payment.vatAmount)}</td><td className="insolvency-claims-overview__amount insolvency-claims-overview__amount--gross">{formatCurrency(payment.grossAmount)}</td>{canEdit && <Actions disabled={Boolean(draft) || saving} onEdit={() => startEdit('payment', payment)} onDelete={() => remove('payment', payment)} />}</>}</tr>
        })}
        {canEdit && draftPayment && draft.mode === 'new' && <tr className="insolvency-claims-overview__row--editing"><PaymentFields values={draft.values} onChange={updateDraft} /><Actions editing disabled={saving} onSave={saveDraft} onCancel={() => setDraft(null)} /></tr>}
      </tbody></table></div>
    </section>
    <div className="insolvency-claims-overview__summary" aria-label="Finanzielle Zusammenfassung"><div><span>Angemeldete Forderungen brutto</span><strong>{formatCurrency(registeredClaims)}</strong></div><div><span>Gesamte offene Forderungen brutto</span><strong>{formatCurrency(totalClaimsGross)}</strong></div><div><span>Enthaltene USt.</span><strong>{formatCurrency(totalClaimsVat)}</strong></div><div><span>Quotenzahlungen brutto</span><strong>{formatCurrency(quotaPaymentsGross)}</strong></div><div><span>Voraussichtlicher BPL-Verlust netto</span><strong className={estimatedLossNet > 0 ? 'insolvency-claims-overview__amount--loss' : ''}>{formatCurrency(estimatedLossNet)}</strong></div></div>
  </section>
}
