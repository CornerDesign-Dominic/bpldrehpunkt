...
export default function NewsList({ items, loading, readItemIds, laterItemIds, favoriteItemIds, reactionsByItem, reactionUpdatingIds, onMarkRead, onToggleLater, onToggleFavorite, onToggleReaction, onEdit, onArchive, onHide, canEdit }) {
  ...
  return <div className="news-list" aria-label="News-Liste">{items.map((item) => {
    ...
    return <article id={`news-${item.id}`} className={unread ? 'news-entry news-entry--unread' : 'news-entry'} key={item.id} data-countries={item.affectedCountries?.join(',') || ''} data-tags={item.topicTags?.join(',') || ''} data-affects={item.affects?.join(',') || ''}>
      <div className="news-entry__meta-column"><time className="news-entry__date">{formatNewsDate(item.publishedAt)}</time><div className="news-entry__meta-flags"><span className={`news-priority news-priority--${item.priority}`}>{getNewsPriority(item.priority)}</span>{item.sourceType === 'internal' && <span className="news-entry__internal">Intern</span>}</div></div>
