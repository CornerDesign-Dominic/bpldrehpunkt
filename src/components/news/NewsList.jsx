...
export default function NewsList({ items, loading, readItemIds, laterItemIds, favoriteItemIds, reactionsByItem, reactionUpdatingIds, onMarkRead, onToggleLater, onToggleFavorite, onToggleReaction, onEdit, onArchive, onHide, canEdit }) {
  ...
  return <div className="news-list" aria-label="News-Liste">{items.map((item) => {
    ...
    return <article id={`news-${item.id}`} className={entryClassName} key={item.id} data-countries={item.affectedCountries?.join(',') || ''} data-tags={item.topicTags?.join(',') || ''} data-affects={item.affects?.join(',') || ''}>
      <div className="news-entry__meta-column"><time className="news-entry__date">{formatNewsDate(item.publishedAt)}</time><div className="news-entry__meta-flags"><span className={`news-priority news-priority--${priority}`}>{priority === 'important' && <span className="news-priority__icon" aria-hidden="true">!</span>}{getNewsPriority(priority)}</span>{item.sourceType === 'internal' && <span className="news-entry__internal">Intern</span>}</div></div>
      <div className="news-entry__content"><div className="news-entry__heading"><h2 title={item.title}>{compactTitle(item)}</h2></div><p className="news-entry__meta">{getNewsCategory(item.category)}{item.sourceType === 'internal' && ` · ${getInternalNewsCategory(item.internalCategory)}`}{item.source ? ` · ${item.source}` : ''}</p><p className="news-entry__summary">{item.aiSummary || item.summary}</p>{reactionsEnabled && <div className="news-entry__reactions" aria-label="News-Reaktionen"><span>Hilfreich?</span><button className={reaction === 'helpful' ? 'news-entry__reaction news-entry__reaction--active' : 'news-entry__reaction'} type="button" aria-label="Hilfreich" aria-pressed={reaction === 'helpful'} title="Hilfreich" disabled={reactionUpdatingIds.has(item.id)} onClick={() => onToggleReaction(item, 'helpful')}><span aria-hidden="true">👍</span><b>{counts.helpful}</b></button><button className={reaction === 'notHelpful' ? 'news-entry__reaction news-entry__reaction--active' : 'news-entry__reaction'} type="button" aria-label="Nicht hilfreich" aria-pressed={reaction === 'notHelpful'} title="Nicht hilfreich" disabled={reactionUpdatingIds.has(item.id)} onClick={() => onToggleReaction(item, 'notHelpful')}><span aria-hidden="true">👎</span><b>{counts.notHelpful}</b></button></div>}</div>
      <div className="news-entry__markers" aria-label="Persönliche Merker"><button className={later ? 'news-entry__marker news-entry__marker--active' : 'news-entry__marker'} type="button" aria-label={later ? 'Später lesen entfernen' : 'Für später lesen markieren'} aria-pressed={later} title={later ? 'Später lesen entfernen' : 'Später lesen'} onClick={() => onToggleLater(item.id)}><svg className="news-entry__bookmark" aria-hidden="true" viewBox="0 0 16 16"><path d="M4 2.5h8v11l-4-2.6-4 2.6z" /></svg></button><button className={favorite ? 'news-entry__marker news-entry__marker--active' : 'news-entry__marker'} type="button" aria-label={favorite ? 'Favorit entfernen' : 'Als Favorit markieren'} aria-pressed={favorite} title={favorite ? 'Favorit entfernen' : 'Favorit'} onClick={() => onToggleFavorite(item.id)}><span aria-hidden="true">{favorite ? '★' : '☆'}</span></button></div>
      <button className="news-entry__toggle" type="button" aria-expanded={expanded} aria-controls={`news-details-${item.id}`} onClick={() => toggle(item)}><svg aria-hidden="true" viewBox="0 0 16 16"><path d={expanded ? 'm4 10 4-4 4 4' : 'm4 6 4 4 4-4'} /></svg><span className="sr-only">{expanded ? 'Meldung einklappen' : 'Meldung aufklappen'}</span></button>
      {expanded && <div className="news-entry__expanded" id={`news-details-${item.id}`}>{(item.affectedCountries?.length || item.topicTags?.length || item.affects?.length) && <div className="news-entry__labels" aria-label="Kennzeichnungen">{item.affectedCountries?.map((country) => <span className="news-entry__label" key={`country-${country}`}>{country}</span>)}{item.topicTags?.map((tag) => <span className="news-entry__label" key={`tag-${tag}`}>{getExternalNewsTag(tag)}</span>)}{item.affects?.map((area) => <span className="news-entry__label news-entry__label--affects" key={`affects-${area}`}>Betrifft: {getExternalNewsAffects(area)}</span>)}</div>}{content && <p className="news-entry__details">{content}</p>}{updates?.loading && <p className="news-entry__updates-loading">Updates werden geladen …</p>}{updates?.items?.length > 0 && <section className="news-entry__updates" aria-label="Update-Historie"><h3>Updates</h3><ol>{updates.items.map((update) => <li key={update.id}><time>{formatNewsDate(update.changedAt)}</time><span>{update.summary}</span>{update.sourceUrl && <a href={update.sourceUrl} target="_blank" rel="noreferrer">Quelle öffnen</a>}</li>)}</ol></section>}{nextStep && <p className="news-entry__next-step"><strong>Nächster Schritt</strong>{nextStep}</p>}<div className="news-entry__actions">{item.sourceUrl && <a className="news-entry__source-link" href={item.sourceUrl} target="_blank" rel="noreferrer">Quelle öffnen <span aria-hidden="true">↗</span></a>}{canEdit && item.sourceType === 'internal' && <button type="button" onClick={() => onEdit(item)}>Bearbeiten</button>}{canEdit && <button className="news-entry__archive" type="button" onClick={() => item.sourceType === 'external' ? onHide(item) : onArchive(item)}>Archivieren</button>}</div></div>}
    </article>
  })}</div>
}
