...
import { createEmptyInkassoMovement } from '../../lib/inkasso.js'
...
export default function InkassoFinancialOverview({ canEdit, embedded = false, inkassoCase, loading, movements, onDelete, onSave }) {
  ...
  const costMovements = movements.filter((movement) => movement.type === 'collection_costs')
  ...
  return <Wrapper {...wrapperProps} aria-labelledby="inkasso-financial-overview-title">
    ...
    <div className="todos-table-frame damage-financial-overview__table-frame"><table className="data-table todos-table damage-financial-overview__table"><colgroup><col className="damage-financial-overview__date-column" /><col className="damage-financial-overview__movement-column" /><col className="damage-financial-overview__amount-column" />{canEdit && <col className="damage-financial-overview__actions-column" />}</colgroup><thead><tr><th>Datum</th><th>Bewegung</th><th>Betrag</th>{canEdit && <th className="damage-financial-overview__actions"><span className="sr-only">Aktionen</span></th>}</tr></thead><tbody>
      {loading ? <tr><td className="table-state" colSpan={columns}>Inkassogebühren werden geladen …</td></tr> : !costMovements.length && draft?.mode !== 'new' ? <tr><td className="table-state inkasso-empty-state" colSpan={columns}>Noch keine Inkassogebühren hinterlegt.</td></tr> : costMovements.map((movement) => {
        const editing = draft?.mode === 'edit' && draft.movement.id === movement.id
        return <tr className={editing ? 'damage-financial-overview__row--editing' : ''} key={movement.id}>{editing ? <><MovementFields draft={draft.values} onChange={updateDraft} /><td className="damage-financial-overview__actions"><button className="damage-financial-overview__icon-action damage-financial-overview__icon-action--save" type="button" disabled={saving} onClick={saveDraft} aria-label="Speichern" title="Speichern"><CheckIcon size={15} /></button><button className="damage-financial-overview__icon-action damage-financial-overview__icon-action--cancel" type="button" disabled={saving} onClick={() => setDraft(null)} aria-label="Abbrechen" title="Abbrechen"><CloseIcon size={15} /></button></td></> : <><td>{formatDate(movement.date)}</td><td><span className="damage-financial-overview__movement">Inkassogebühr{movement.description ? ` · ${movement.description}` : ''}</span></td><td>{formatCurrency(movement.amount)}</td>{canEdit && <td className="damage-financial-overview__actions"><button className="damage-financial-overview__icon-action" type="button" disabled={Boolean(draft) || saving} onClick={() => { setError(''); setDraft({ mode: 'edit', movement, values: { date: movement.date, type: 'collection_costs', amount: movement.amount, description: movement.description || '' } }) }} aria-label="Bearbeiten" title="Bearbeiten"><EditIcon size={15} /></button><button className="damage-financial-overview__icon-action damage-financial-overview__icon-action--delete" type="button" disabled={Boolean(draft) || saving} onClick={() => remove(movement)} aria-label="Löschen" title="Löschen"><TrashIcon size={15} /></button></td>}</>}</tr>
      })}
