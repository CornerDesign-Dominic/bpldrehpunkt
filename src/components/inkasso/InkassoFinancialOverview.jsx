...
function MovementFields({ draft, onChange }) {
  return <><td><input aria-label="Datum" type="date" value={draft.date} onChange={(event) => onChange('date', event.target.value)} /></td><td><div className="inkasso-financial-overview__movement-editor"><select aria-label="Bewegungsart" value={draft.type} onChange={(event) => onChange('type', event.target.value)}>{INKASSO_FINANCIAL_COST_TYPES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><input aria-label="Hinweis" value={draft.description || ''} maxLength="500" placeholder="Hinweis" onChange={(event) => onChange('description', event.target.value)} /></div></td><td><input aria-label="Betrag" type="number" min="0" step="0.01" value={draft.amount} onChange={(event) => onChange('amount', event.target.value)} /></td></>
}
...
export default function InkassoFinancialOverview({ canEdit, embedded = false, inkassoCase, loading, movements, onDelete, onSave }) {
  ...
  const totalCosts = costMovements.reduce((total, movement) => total + Number(movement.amount || 0), 0)
  ...
  return <Wrapper {...wrapperProps} aria-labelledby="inkasso-financial-overview-title">
    <div className="todo-detail-section-heading"><h3 id="inkasso-financial-overview-title">Finanzieller Überblick</h3>{canEdit && <button className="button damage-financial-overview__add" type="button" disabled={Boolean(draft) || saving} onClick={() => { setError(''); setDraft({ mode: 'new', values: { ...createEmptyInkassoMovement(), type: 'collection_costs' } }) }}>Bewegung hinzufügen</button>}</div>
    {error && <p className="form-error">{error}</p>}
    <div className="todos-table-frame damage-financial-overview__table-frame"><table className="data-table todos-table damage-financial-overview__table"><colgroup><col className="damage-financial-overview__date-column" /><col className="damage-financial-overview__movement-column" /><col className="damage-financial-overview__amount-column" />{canEdit && <col className="damage-financial-overview__actions-column" />}</colgroup><thead><tr><th>Datum</th><th>Bewegung</th><th>Betrag</th>{canEdit && <th className="damage-financial-overview__actions"><span className="sr-only">Aktionen</span></th>}</tr></thead><tbody>
    ...
    <div className="inkasso-financial-overview__summary"><div><span>Gebühren &amp; Kosten</span><strong>{formatCurrency(totalCosts)}</strong></div><div><span>Zahlungen vom Schuldner</span><strong>{formatCurrency(paidAmount)}</strong></div></div>
  </Wrapper>
}
