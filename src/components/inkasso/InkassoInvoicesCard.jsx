...
export default function InkassoInvoicesCard({ canEdit, invoices, inkassoCase, loading, movements, movementsLoading, onDeleteMovement, onPaymentChange, onSave, onSaveMovement, savingInvoiceId }) {
  ...
  return <section className="todo-detail-content damage-financial-overview inkasso-invoices-card" aria-labelledby="inkasso-invoices-title">
    ...
    <div className="todo-detail-section-heading"><h3 id="inkasso-invoices-title">Rechnungen &amp; Forderungen</h3>{canEdit && <button className="button damage-financial-overview__add" type="button" disabled={loading || Boolean(draft) || saving || Boolean(savingInvoiceId)} onClick={() => { setError(''); setDraft({ invoiceNumber: '', netAmount: '', vatAmount: '' }) }}>Rechnung hinzufügen</button>}</div>
    {error && <p className="form-error">{error}</p>}
    <div className="damage-cases-table-frame inkasso-invoices-card__table-frame"><table className="data-table todos-table"><thead><tr><th>Rechnungsnummer</th><th>Netto</th><th>USt.</th><th>Gesamt</th><th>Bezahlt</th></tr></thead><tbody>
