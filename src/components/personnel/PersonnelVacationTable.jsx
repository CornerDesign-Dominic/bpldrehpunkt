...
export function PersonnelVacationTable({ vacations, includeEmployee = true, editable = false, onEdit }) {
  ...
  return <div className={`personnel-vacation-table personnel-vacation-table--${includeEmployee ? 'with-employee' : 'detail'} table-frame`}><table><thead><tr>{includeEmployee && <><th>Mitarbeiter</th><th>Abteilung</th></>}<th>Von</th><th>Bis</th><th>Tage</th><th>Urlaubsart</th><th>Status</th><th>Lohnbuchhaltung</th><th>HR-Bemerkung</th></tr></thead><tbody>
    {vacations.length ? vacations.map((vacation) => <tr className={`${vacation.status === 'approved' && !vacation.payrollProcessed ? 'personnel-vacation-table__row--open' : ''}${editable ? ' personnel-vacation-table__row--link' : ''}`} key={vacation.vacationId} role={editable ? 'button' : undefined} tabIndex={editable ? 0 : undefined} aria-label={editable ? `HR-Informationen für Urlaub vom ${formatPersonnelVacationDate(vacation.startDate)} pflegen` : undefined} onClick={editable ? () => onEdit(vacation) : undefined} onKeyDown={editable ? (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onEdit(vacation) } } : undefined}>{includeEmployee && <><td><strong>{vacation.employeeName}</strong></td><td>{vacation.department}</td></>}<td>{formatPersonnelVacationDate(vacation.startDate)}</td><td>{formatPersonnelVacationDate(vacation.endDate)}</td><td>{vacation.days}</td><td>{vacationTypeLabels[vacation.vacationType] || 'Normal'}</td><td><span className={`personnel-vacation-status personnel-vacation-status--${vacation.status}`}>{vacationStatusLabels[vacation.status] || 'Ausstehend'}</span></td><td><span className={vacation.payrollProcessed ? 'personnel-payroll personnel-payroll--done' : 'personnel-payroll personnel-payroll--open'}>{vacation.payrollProcessed ? 'Erledigt' : 'Offen'}</span></td><td className="personnel-vacation-table__note" title={vacation.hrNote || undefined}>{vacation.hrNote || '—'}</td></tr>) : <tr><td colSpan={columns} className="table-state">Keine Urlaube für diese Auswahl vorhanden.</td></tr>}
  </tbody></table></div>
}
