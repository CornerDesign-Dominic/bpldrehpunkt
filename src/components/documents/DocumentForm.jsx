...
export default function DocumentForm({ documentItem, hideExpirationDate = false, onCancel, onSubmit }) {
  ...
  return <form className="document-form" onSubmit={submit} noValidate>
    ...
    <div className="document-form__grid">
      ...
      {!hideExpirationDate && <label className="form-field"><span>Gültig bis <small>(optional)</small></span><input type="date" value={form.expirationDate} onChange={(event) => update('expirationDate', event.target.value)} /></label>}
