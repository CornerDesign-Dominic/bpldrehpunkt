...
function NodeCard({ incomingCounts, node, editable, onAdd, onConnect, onDelete, onDisconnect, onEdit, onInsert, process, visited }) {
  ...
  return <div className="process-flow__node-wrap">
    ...
    {hasLoop ? <p className="process-flow__invalid">Zirkuläre Verbindung</p> : <div className={node.type === 'decision' ? `process-flow__branches process-flow__branches--decision${branches.length > 2 ? ' process-flow__branches--multiple' : ''}` : 'process-flow__branches'}>{branches.map((branch) => {
      ...
      return <div className="process-flow__branch" key={branch.key}>
        ...
        {target ? <>{incomingCounts[target.id] > 1 && <span className="process-flow__merge-hint">führt zu gemeinsamem Schritt</span>}{editable && <div className="process-flow__connection-actions">{node.type === 'decision' && <><button type="button" onClick={() => onConnect(node, branch.key)}>Verbindung ändern</button><button type="button" onClick={() => onDisconnect(node, branch.key)}>Verbindung lösen</button></>}<button type="button" className="process-flow__insert" onClick={() => onInsert(branch.edge)}>＋ Schritt dazwischen einfügen</button></div>}<NodeCard incomingCounts={incomingCounts} node={target} editable={editable} onAdd={onAdd} onConnect={onConnect} onDisconnect={onDisconnect} onEdit={onEdit} onDelete={onDelete} onInsert={onInsert} process={process} visited={nextVisited} /></> : editable && node.type !== 'end' ? <div className="process-flow__open-actions"><button type="button" className="process-flow__add" onClick={() => onAdd(node, branch.key)}>Nächsten Schritt hinzufügen</button>{node.type === 'decision' && <button type="button" className="process-flow__connect" onClick={() => onConnect(node, branch.key)}>Mit bestehendem Schritt verbinden</button>}</div> : <span className="process-flow__open-end">Noch nicht ergänzt</span>}
      </div>
    })}</div>}
  </div>
}

export default function ProcessFlow({ editable = false, onAdd, onConnect, onDelete, onDisconnect, onEdit, onInsert, process }) {
  ...
  return <section className="process-flow" aria-label="Prozessablauf"><NodeCard incomingCounts={incomingCounts} node={start} editable={editable} onAdd={onAdd} onConnect={onConnect} onDisconnect={onDisconnect} onEdit={onEdit} onDelete={onDelete} onInsert={onInsert} process={process} visited={new Set()} /></section>
}
