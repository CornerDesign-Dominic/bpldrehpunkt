...
import { createEmptyDamageDeadline, damageDeadlinePresentation } from '../../lib/damages.js'
  ...
  return <div className="todo-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) onClose() }}>
    <section className="todo-quick-edit-modal damage-deadline-modal" role="dialog" aria-modal="true" aria-labelledby="damage-deadline-modal-title">
      <form className="todo-quick-editor" onSubmit={submit} noValidate>
        <div className="todo-quick-editor__heading damage-deadline-modal__heading"><h2 id="damage-deadline-modal-title">{title}</h2>{canEdit && mode !== 'new' && !editing && <button className="todo-detail-section-edit" type="button" onClick={beginEditing} title="Termin bearbeiten" aria-label="Termin bearbeiten"><EditIcon size={14} /></button>}</div>
        {editing ? <div className="todo-quick-editor__grid damage-deadline-modal__fields">
          ...
          <section><h3>Termininformationen</h3><dl><div><dt>Datum</dt><dd><span className={deadlineClass(deadline)}>{formatDate(deadline.date)} · {damageDeadlinePresentation(deadline).label}</span></dd></div><div><dt>Erinnerung</dt><dd><span className={deadline.reminderEnabled ? 'damage-deadlines__reminder damage-deadlines__reminder--on' : 'damage-deadlines__reminder'}>{deadline.reminderEnabled ? 'An' : 'Aus'}</span></dd></div><div><dt>Bemerkung</dt><dd className="damage-deadline-modal__note-value">{deadline.note || '—'}</dd></div></dl></section>
