...
export default function DamageFinancialOverview({ canEdit, loading, movements, onDelete, onSave }) {
  ...
  return <section className="todo-detail-content damage-financial-overview" aria-labelledby="damage-financial-overview-title">
    ...
    <div className="todos-table-frame damage-financial-overview__table-frame"><table className="data-table todos-table damage-financial-overview__table"><colgroup><col className="damage-financial-overview__date-column" /><col className="damage-financial-overview__movement-column" /><col className="damage-financial-overview__amount-column" />{canEdit && <col className="damage-financial-overview__actions-column" />}</colgroup><thead><tr><th>Datum</th><th>Bewegung</th><th>Betrag</th>{canEdit && <th className="damage-financial-overview__actions"><span className="sr-only">Aktionen</span></th>}</tr></thead><tbody>
      {loading ? <tr><td className="table-state" colSpan={columns}>Betragsbewegungen werden geladen …</td></tr> : !movements.length && draft?.mode !== 'new' ? <tr><td className="table-state damage-financial-overview__empty-state" colSpan={columns}>Noch keine Betragsbewegungen hinterlegt.</td></tr> : movements.map((movement) => {
        const isEditing = draft?.mode === 'edit' && draft.movement.id === movement.id
        return <tr className={isEditing ? 'damage-financial-overview__row--editing' : ''} key={movement.id}>{isEditing ? <><MovementFields draft={draft.values} onChange={updateDraft} /><td className="damage-financial-overview__actions"><button className="damage-financial-overview__icon-action damage-financial-overview__icon-action--save" type="button" disabled={saving} onClick={saveDraft} aria-label="Speichern" title="Speichern"><CheckIcon size={15} /></button><button className="damage-financial-overview__icon-action damage-financial-overview__icon-action--cancel" type="button" disabled={saving} onClick={() => setDraft(null)} aria-label="Abbrechen" title="Abbrechen"><CloseIcon size={15} /></button></td></> : <><td>{formatDate(movement.date)}</td><td><span className="damage-financial-overview__movement">{labelFor(DAMAGE_MOVEMENT_TRANSACTION_TYPES, movement.transactionType)} {labelFor(DAMAGE_MOVEMENT_COUNTERPARTIES, movement.counterpartyType)}</span></td><td className={`damage-financial-overview__value damage-financial-overview__value--${movement.transactionType}`}>{formatCurrency(movement.amount)}</td>{canEdit && <td className="damage-financial-overview__actions"><button className="damage-financial-overview__icon-action" type="button" disabled={Boolean(draft) || saving} onClick={() => startEdit(movement)} aria-label="Bearbeiten" title="Bearbeiten"><EditIcon size={15} /></button><button className="damage-financial-overview__icon-action damage-financial-overview__icon-action--delete" type="button" disabled={Boolean(draft) || saving} onClick={() => deleteMovement(movement)} aria-label="Löschen" title="Löschen"><TrashIcon size={15} /></button></td>}</>}</tr>
      })}
