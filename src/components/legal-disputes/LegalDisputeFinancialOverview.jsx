...
function EntryFields({ draft, onChange }) {
  const paying = draft.direction === 'paid'
  return <>
    <td><input aria-label="Zahlungsdatum" type="date" value={draft.date} onChange={(event) => onChange('date', event.target.value)} /></td>
    <td><div className="legal-dispute-financial-overview__edit-entry"><select aria-label="Zahlungsrichtung" value={draft.direction} onChange={(event) => onChange('direction', event.target.value)}>{LEGAL_DISPUTE_FINANCIAL_DIRECTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>{paying && <select aria-label="Zahlungsempfänger" value={draft.payeeType} onChange={(event) => onChange('payeeType', event.target.value)}><option value="">Empfänger wählen</option>{LEGAL_DISPUTE_PAYMENT_RECIPIENTS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>}</div></td>
...
export default function LegalDisputeFinancialOverview({ canEdit, entries, legalDispute, loading, onDeleteEntry, onEditDispute, onSaveEntry }) {
  ...
  function updateDraft(field, value) { setDraft((current) => ({ ...current, values: { ...current.values, [field]: value, ...(field === 'direction' && value === 'received' ? { payeeType: '' } : {}) } })); setError('') }
  ...
  return <section className="todo-detail-content damage-financial-overview legal-dispute-financial-overview" aria-labelledby="legal-dispute-financial-overview-title">
    <div className="todo-detail-section-heading"><h3 id="legal-dispute-financial-overview-title">Finanzieller Überblick</h3>{canEdit && <div className="legal-dispute-financial-overview__actions"><button className="button button--secondary damage-financial-overview__add" type="button" disabled={Boolean(draft) || saving} onClick={onEditDispute}>Streitbetrag bearbeiten</button><button className="button damage-financial-overview__add" type="button" disabled={Boolean(draft) || saving} onClick={startNew}>Zahlung hinzufügen</button></div>}</div>
    ...
    <div className="todos-table-frame damage-financial-overview__table-frame"><table className="data-table todos-table damage-financial-overview__table legal-dispute-financial-overview__table"><thead><tr><th>Datum</th><th>IST-Zahlung</th><th>Netto</th><th>USt.</th><th>Brutto</th>{canEdit && <th className="damage-financial-overview__actions"><span className="sr-only">Aktionen</span></th>}</tr></thead><tbody>
      {loading ? <tr><td className="table-state" colSpan={columns}>Zahlungspositionen werden geladen …</td></tr> : !entries.length && draft?.mode !== 'new' ? <tr><td className="table-state" colSpan={columns}>Noch keine Ist-Zahlungen hinterlegt.</td></tr> : entries.map((entry) => {
        ...
        const recipientLabel = entry.direction === 'paid' ? labelFor(LEGAL_DISPUTE_PAYMENT_RECIPIENTS, entry.payeeType) : ''
        return <tr className={editing ? 'damage-financial-overview__row--editing' : ''} key={entry.id}>{editing ? <><EntryFields draft={draft.values} onChange={updateDraft} /><td className="damage-financial-overview__actions"><button className="damage-financial-overview__icon-action damage-financial-overview__icon-action--save" type="button" disabled={saving} onClick={saveDraft} aria-label="Speichern" title="Speichern"><CheckIcon size={15} /></button><button className="damage-financial-overview__icon-action damage-financial-overview__icon-action--cancel" type="button" disabled={saving} onClick={() => setDraft(null)} aria-label="Abbrechen" title="Abbrechen"><CloseIcon size={15} /></button></td></> : <><td>{formatDate(entry.date)}</td><td><span className={`legal-dispute-financial-overview__direction legal-dispute-financial-overview__direction--${entry.direction}`}>{directionLabel}{recipientLabel ? ` · ${recipientLabel}` : ''}</span></td><td>{formatCurrency(entry.netAmount)}</td><td>{formatCurrency(entry.vatAmount)}</td><td className={entry.direction === 'received' ? 'damage-financial-overview__amount--positive' : 'damage-financial-overview__amount--negative'}>{formatCurrency(grossAmount)}</td>{canEdit && <td className="damage-financial-overview__actions"><button className="damage-financial-overview__icon-action" type="button" disabled={Boolean(draft) || saving} onClick={() => startEdit(entry)} aria-label="Bearbeiten" title="Bearbeiten"><EditIcon size={15} /></button><button className="damage-financial-overview__icon-action damage-financial-overview__icon-action--delete" type="button" disabled={Boolean(draft) || saving} onClick={() => deleteEntry(entry)} aria-label="Löschen" title="Löschen"><TrashIcon size={15} /></button></td>}</>}</tr>
      })}
