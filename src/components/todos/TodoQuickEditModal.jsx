...
function initialValues(todo, currentUserId) {
  ...
  return {
    ...
    damageCaseId: todo.damageCaseId || '',
...
export default function TodoQuickEditModal({ canViewDamageCases = false, currentUserId, damageCases = [], onCancel, onSubmit, partners = [], section, todo, users = [] }) {
  ...
  return <div className="todo-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel() }}>
    <section className="todo-quick-edit-modal" role="dialog" aria-modal="true" aria-labelledby="todo-quick-edit-title">
      <form className="todo-quick-editor" onSubmit={save} noValidate>
        ...
        {section === 'content' && <div className="todo-quick-editor__grid"><label className="form-field"><span>Titel *</span><input value={form.title} onChange={(event) => update('title', event.target.value)} autoFocus /></label><label className="form-field"><span>Beschreibung</span><textarea rows="6" value={form.description} onChange={(event) => update('description', event.target.value)} /></label></div>}
        {section === 'schedule' && <div className="todo-quick-editor__grid todo-quick-editor__grid--three"><TodoPriorityPicker value={form.priority} onChange={(value) => update('priority', value)} /><label className="form-field"><span>Fällig am</span><input type="date" value={form.dueDate} onChange={(event) => update('dueDate', event.target.value)} /></label><label className="form-field"><span>Erinnerung am</span><input type="date" value={form.reminderDate} onChange={(event) => update('reminderDate', event.target.value)} /></label></div>}
        {section === 'responsibility' && <div className="todo-quick-editor__grid"><label className="form-field"><span>Aufgabe für</span><select value={form.audienceType} onChange={(event) => changeAudienceType(event.target.value)}><option value="self">Für mich</option><option value="department">Abteilung</option><option value="all">Alle</option><option value="people">Person/en</option></select></label>{form.audienceType === 'department' && <label className="form-field"><span>Abteilung</span><select value={form.audienceId} onChange={(event) => update('audienceId', event.target.value)}><option value="">Bitte wählen</option>{departments.map((department) => <option key={department} value={department}>{department}</option>)}</select></label>}{form.audienceType === 'people' && <fieldset className="todo-quick-editor__people"><legend>Person/en *</legend><div>{users.filter((user) => user.id !== currentUserId).map((user) => <label className={`todo-quick-editor__person${form.audienceIds.includes(user.id) ? ' todo-quick-editor__person--selected' : ''}`} key={user.id}><input type="checkbox" checked={form.audienceIds.includes(user.id)} onChange={() => togglePerson(user.id)} /><span>{getUserDisplayName(user, user)}</span></label>)}</div></fieldset>}{form.audienceType === 'self' && <p className="todo-quick-editor__hint">Die Aufgabe wird Ihnen direkt zugewiesen.</p>}{form.audienceType === 'all' && <p className="todo-quick-editor__hint">Alle aktiven Nutzer können die Aufgabe sehen und übernehmen.</p>}</div>}
        {section === 'links' && <div className="todo-quick-editor__grid"><label className="form-field"><span>Kunde</span><select value={form.customerId} onChange={(event) => changePartner('customer', event)}><option value="">Kein Kunde verknüpft</option>{form.customerId && !customers.some((partner) => partner.id === form.customerId) && <option value={form.customerId}>{form.customerName || 'Verknüpfter Kunde'}</option>}{customers.map((partner) => <option key={partner.id} value={partner.id}>{partner.companyName}</option>)}</select></label><label className="form-field"><span>Unternehmer</span><select value={form.carrierId} onChange={(event) => changePartner('carrier', event)}><option value="">Kein Unternehmer verknüpft</option>{form.carrierId && !carriers.some((partner) => partner.id === form.carrierId) && <option value={form.carrierId}>{form.carrierName || 'Verknüpfter Unternehmer'}</option>}{carriers.map((partner) => <option key={partner.id} value={partner.id}>{partner.companyName}</option>)}</select></label><label className="form-field"><span>TA-Nummer</span><input value={form.reference} maxLength="240" onChange={(event) => update('reference', event.target.value)} placeholder="Auftrags-, Tour- oder Rechnungsnummer" /></label>{canViewDamageCases && <label className="form-field"><span>Schadenfall suchen</span><input value={damageCaseSearch} onChange={(event) => setDamageCaseSearch(event.target.value)} placeholder="Nummer, Titel oder Referenz" /></label>}<label className="form-field"><span>Schadenfall</span><select value={form.damageCaseId} onChange={(event) => update('damageCaseId', event.target.value)}><option value="">Kein Schadenfall verknüpft</option>{form.damageCaseId && !damageCases.some((damageCase) => damageCase.id === form.damageCaseId) && <option value={form.damageCaseId}>Schadenfall nicht verfügbar</option>}{canViewDamageCases && form.damageCaseId && !matchingDamageCases.some((damageCase) => damageCase.id === form.damageCaseId) && damageCases.find((damageCase) => damageCase.id === form.damageCaseId) && <option value={form.damageCaseId}>{damageCaseLabel(damageCases.find((damageCase) => damageCase.id === form.damageCaseId))}</option>}{canViewDamageCases && matchingDamageCases.map((damageCase) => <option key={damageCase.id} value={damageCase.id}>{damageCaseLabel(damageCase)}{damageCase.transportReference ? ` · ${damageCase.transportReference}` : ''}</option>)}</select></label>{canViewDamageCases && damageCaseSearch.trim() && !matchingDamageCases.length && <p className="todo-quick-editor__hint">Kein passender Schadenfall gefunden.</p>}</div>}
        {error && <p className="form-error">{error}</p>}
        <div className="form-actions"><button className="button button--secondary" type="button" onClick={onCancel}>Abbrechen</button><button className="button" type="submit" disabled={saving}>{saving ? 'Wird gespeichert …' : 'Speichern'}</button></div>
      </form>
    </section>
  </div>
}
