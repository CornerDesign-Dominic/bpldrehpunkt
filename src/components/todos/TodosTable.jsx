...
function PriorityChip({ priority }) { const symbol = priority === 'high' ? '!' : priority === 'medium' ? '•' : null; return <span className={`todo-priority todo-priority--${priority}`}><span className="todo-priority__icon" aria-hidden="true">{symbol || <TodoPriorityIcon priority="low" />}</span>{TODO_PRIORITY[priority]}</span> }
