...
export default function TodosTable({ formatDate, onOpen, todos }) {
  ...
  return <div className="todos-list-table-frame"><table className="data-table todos-table"><thead><tr><SortableHeader activeSort={sort} label="Status" onSort={toggleSort} sortKey="status" /><SortableHeader activeSort={sort} label="Aufgabe" onSort={toggleSort} sortKey="title" /><SortableHeader activeSort={sort} label="Wichtigkeit" onSort={toggleSort} sortKey="priority" /><SortableHeader activeSort={sort} label="Fällig am" onSort={toggleSort} sortKey="dueDate" /><SortableHeader activeSort={sort} label="Bearbeiter" onSort={toggleSort} sortKey="assignee" /><SortableHeader activeSort={sort} label="Erstellt von" onSort={toggleSort} sortKey="creator" /><SortableHeader activeSort={sort} label="Zuständigkeit" onSort={toggleSort} sortKey="responsibility" /></tr></thead><tbody>{visibleTodos.map((todo) => { const due = todoDuePresentation(todo); const appearance = due.days === null ? 'none' : due.days <= 0 ? 'critical' : due.days <= 2 ? 'urgent' : due.days <= 5 ? 'warning' : 'none'; const dueText = !todo.dueDate ? 'Keine Fälligkeit' : `${due.label} · ${formatDate(todo.dueDate)}`; return <tr className={`todos-table__row todos-table__row--${appearance}`} key={todo.id} tabIndex="0" role="link" aria-label={`To-do öffnen: ${todo.title}`} onClick={() => onOpen(todo)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onOpen(todo) } }}><td><StatusChip status={todoStatus(todo)} /></td><td className="todos-table__title" title={todo.title}>{todo.title}</td><td><TodoPriority priority={todoPriority(todo)} /></td><td className={`todos-table__due todos-table__due--${appearance}`}>{dueText}</td><td>{todo.assignedUserName || 'Noch nicht übernommen'}</td><td>{todo.creatorName || '—'}</td><td>{responsibilityLabel(todo)}</td></tr> })}</tbody></table></div>
}
