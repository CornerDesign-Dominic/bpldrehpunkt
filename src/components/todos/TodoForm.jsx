...
function initialValues(todo, currentUserId, fixedLink) {
  ...
  const values = {
    ...
    customerId: todo.customerId || '', customerName: todo.customerName || '', carrierId: todo.carrierId || '', carrierName: todo.carrierName || '', reference: todo.reference || '', damageCaseId: todo.damageCaseId || '', insolvencyId: todo.insolvencyId || '', legalDisputeId: todo.legalDisputeId || '', inkassoCaseId: todo.inkassoCaseId || '',
...
export default function TodoForm({ currentUserId, fixedLink = null, initialTodo, onCancel, onSubmit, partners = [], users }) {
  ...
  return <form className="todo-form" onSubmit={submit} noValidate>
    ...
    <section className="todo-form__section"><h3>Terminierung</h3><div className="todo-form__section-grid todo-form__section-grid--schedule"><TodoPriorityPicker value={form.priority} onChange={(value) => update('priority', value)} /><label className="form-field"><span>Fällig am</span><input type="date" value={form.dueDate} onChange={(event) => update('dueDate', event.target.value)} /></label><label className="form-field"><span>Erinnerung am</span><input type="date" value={form.reminderDate} onChange={(event) => update('reminderDate', event.target.value)} /></label></div></section>
    <section className="todo-form__section"><h3>Zuständigkeit</h3><div className="todo-form__section-grid todo-form__section-grid--assignment"><label className="form-field"><span>Aufgabe für</span><select value={form.audienceType} onChange={(event) => changeAudienceType(event.target.value)}><option value="self">Für mich</option><option value="department">Abteilung</option><option value="all">Alle</option><option value="people">Person/en</option></select></label>{form.audienceType === 'department' && <label className="form-field"><span>Abteilung</span><select value={form.audienceId} onChange={(event) => update('audienceId', event.target.value)}><option value="">Bitte wählen</option>{departments.map((department) => <option key={department} value={department}>{department}</option>)}</select></label>}{form.audienceType === 'people' && <fieldset className="todo-form__people"><legend>Person/en *</legend><div className="todo-form__people-options">{users.filter((user) => user.id !== currentUserId).map((user) => <label className={`todo-form__person-option${form.audienceIds.includes(user.id) ? ' todo-form__person-option--selected' : ''}`} key={user.id}><input type="checkbox" checked={form.audienceIds.includes(user.id)} onChange={() => togglePerson(user.id)} /><span>{getUserDisplayName(user, user)}</span></label>)}</div><small>Mehrere Personen können ausgewählt werden.</small></fieldset>}{form.audienceType === 'self' && <p className="todo-form__selection-hint">Die Aufgabe wird Ihnen direkt zugewiesen.</p>}{form.audienceType === 'all' && <p className="todo-form__selection-hint">Alle aktiven Nutzer können die Aufgabe sehen und übernehmen.</p>}</div></section>
    <section className="todo-form__section"><h3>Verknüpfungen</h3><div className="todo-form__section-grid todo-form__section-grid--links">{fixedLink && <label className="form-field"><span>{fixedLink.label}</span><input value={fixedLink.value} readOnly aria-readonly="true" /></label>}<label className="form-field"><span>Kunde</span><select value={form.customerId} onChange={(event) => changePartner('customer', event)}><option value="">Kein Kunde verknüpft</option>{form.customerId && !customers.some((partner) => partner.id === form.customerId) && <option value={form.customerId}>{form.customerName || 'Verknüpfter Kunde'}</option>}{customers.map((partner) => <option key={partner.id} value={partner.id}>{partner.companyName}</option>)}</select></label><label className="form-field"><span>Unternehmer</span><select value={form.carrierId} onChange={(event) => changePartner('carrier', event)}><option value="">Kein Unternehmer verknüpft</option>{form.carrierId && !carriers.some((partner) => partner.id === form.carrierId) && <option value={form.carrierId}>{form.carrierName || 'Verknüpfter Unternehmer'}</option>}{carriers.map((partner) => <option key={partner.id} value={partner.id}>{partner.companyName}</option>)}</select></label><label className="form-field"><span>TA-Nummer</span><input value={form.reference} maxLength="240" onChange={(event) => update('reference', event.target.value)} placeholder="Auftrags-, Tour- oder Rechnungsnummer" /></label></div></section>
